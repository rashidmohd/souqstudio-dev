...
| **E13** Admin panel | **Partly built, 22–23 September.** Staff auth against `admin_users` (sessions in `admin_sessions`, 8h absolute and 1h idle, `ADMIN_SESSION_SECRET` keying the stored hash so rotating it ends every session), an IP allowlist in middleware, three nesting roles, and an audit log written by one module on every mutation. Three surfaces on top: **catalog** (both collections, search and filters, add, edit, archive, restore, promote, plus the image tools: see every photo on a checkerboard, replace one through a presigned PUT, re-run background removal, approve or reject a matte), the **block library console** (list every block, publish one to the shared library and sync it, super admin only), and **AI prompt management** (the 19 `cover_prompts` rows, which moved into the database in September so the art direction could be tuned and until now were editable only with SQL). Plus an overview and the audit log itself. **Not built: E13-01's organization half and impersonation, E13-03's contribution queue, bulk import, synonym editing, enrichment triggers, E13-05, E13-06 and E13-07.** No sign-up and deliberately no bootstrap route: `pnpm --filter @souqstudio/db admin:create`. See `E13-pending.md`. |
