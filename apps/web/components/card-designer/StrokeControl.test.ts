...
import { readPercent } from './StrokeControl'
...
describe('readPercent', () => {
  it('stores a percent as a fraction', () => {
    expect(readPercent('0.4')).toBeCloseTo(0.004, 10)
    expect(readPercent('2')).toBeCloseTo(0.02, 10)
  })

  it('clamps to a border a card can carry', () => {
    // The schema allows a fifth of a card. A fifth of a card is not a border.
    expect(readPercent('99')).toBeCloseTo(0.05, 10)
    expect(readPercent('0')).toBeCloseTo(0.001, 10)
  })
  ...
  it('survives an emptied field and a half-typed one', () => {
    ...
    expect(readPercent('')).toBeCloseTo(0.001, 10)
    expect(readPercent('-')).toBeCloseTo(0.001, 10)
    expect(readPercent('abc')).toBeCloseTo(0.001, 10)
    expect(Number.isFinite(readPercent('1e'))).toBe(true)
  })

  it('never returns a width the document schema would refuse', () => {
    for (const input of ['', 'x', '-5', '0.0001', '1000', '3.7']) {
      const width = readPercent(input)
