...
describe('defaults', () => {
  it('ships at least the minimum', () => {
    expect(defaultTextStyles({}, TEST_CATALOG).length).toBeGreaterThanOrEqual(MIN_STYLES)
  })

  it('names styles for what they are, not how big they are', () => {
    const names = defaultTextStyles({}, TEST_CATALOG).map((s) => s.name)
  ...
  it('binds every slot a seeded block can name', () => {
    const slots = defaultTextStyles({}, TEST_CATALOG).map((s) => s.slot)
  ...
  it('keeps the headline a different face from the product name', () => {
    ...
    const styles = defaultTextStyles(kit, TEST_CATALOG)
  ...
  it('orders the scale so a headline outranks small print', () => {
    const styles = defaultTextStyles({}, TEST_CATALOG)
...
describe('resolveTextStyles', () => {
  it('returns what the kit stores', () => {
    const styles = [custom()]
    expect(resolveTextStyles({ textStyles: styles }, TEST_CATALOG)).toEqual(styles)
  })

  it('falls back to the defaults for a kit that predates styles', () => {
    expect(resolveTextStyles({}, TEST_CATALOG).length).toBe(defaultTextStyles({}, TEST_CATALOG).length)
  })

  it('holds more than the default set — a brand is not capped at eight', () => {
    const many = [...defaultTextStyles({}, TEST_CATALOG), custom({ id: 'a' }), custom({ id: 'b' })]
    expect(resolveTextStyles({ textStyles: many }, TEST_CATALOG)).toHaveLength(10)
  })
})

describe('styleForSlot', () => {
  it('finds the style bound to a slot', () => {
    expect(styleForSlot({}, 'h3', TEST_CATALOG).name).toBe('Product name')
  })
  ...
  it('falls back rather than leaving a block with nothing to render in', () => {
    ...
    expect(styleForSlot(kit, 'h3', TEST_CATALOG).family).toBeTruthy()
  })
})

describe('typographyPatch', () => {
  it('mirrors the bound families back into the four font slots', () => {
    const patch = typographyPatch(defaultTextStyles({ fontHeadline: 'Lalezar' }, TEST_CATALOG))
...
describe('bounds', () => {
  ...
  it('allows removing an owner-added style once above the floor', () => {
    const styles = [...defaultTextStyles({}, TEST_CATALOG), custom()]
  ...
  it('stops adding at the ceiling', () => {
    ...
    expect(canAddStyle(defaultTextStyles({}, TEST_CATALOG))).toBe(true)
  })
})
...
describe('italic', () => {
  it('flags a synthesised slant on a family with no true italic', () => {
    ...
    expect(italicIsSynthetic(custom({ family: 'Cairo', italic: true }), TEST_CATALOG)).toBe(true)
  })

  it('does not flag a family that ships one', () => {
    expect(italicIsSynthetic(custom({ family: 'Rubik', italic: true }), TEST_CATALOG)).toBe(false)
  })

  it('says nothing when italic is off', () => {
    expect(italicIsSynthetic(custom({ family: 'Cairo', italic: false }), TEST_CATALOG)).toBe(false)
  })
})

describe('newTextStyle', () => {
  it('starts from the kit rather than blank', () => {
    const style = newTextStyle({}, defaultTextStyles({}, TEST_CATALOG), TEST_CATALOG)
